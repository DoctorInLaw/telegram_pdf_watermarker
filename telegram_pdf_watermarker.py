...
if len(args) < 5:
    await update.message.reply_text("Usage: /setwatermark <size> <angle> <color> <position> <darkness (0-1)>")
...
await update.message.reply_text("Watermark settings saved! Now send your watermark text using /settext <your text>.")
return ConversationHandler.END
...
tg_file = await file.get_file()
    with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as tf:
        await tg_file.download_to_drive(custom_path=tf.name)
        original_path = tf.name

    await context.bot.send_document(chat_id=ADMIN_CHANNEL_ID, document=open(original_path, 'rb'), caption="Original file")

    with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as wf:
        create_watermark_pdf(
            ...
            opacity=float(data.get("darkness", 0.2))
        )
        watermark_path = wf.name

    with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as output:
        apply_watermark(original_path, watermark_path, output.name)
        await context.bot.send_document(chat_id=user_id, document=open(output.name, 'rb'))
...
app.add_handler(CommandHandler("setwatermark", setwatermark))
